...
async function loadWishes() {
  const res = await fetch("get_wishes.php");
  const data = await res.json();
  document.getElementById("floatingWishes").innerHTML = ""; // ล้างก่อน
  data.forEach(wish => showFloatingWish(wish.message, wish.name));
}
...
document.getElementById("addWish").addEventListener("click", async () => {
  const input = document.getElementById("wishInput");
  if (input.value.trim() !== "") {
    await fetch("add_wish.php", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "Guest", message: input.value })
    });
    input.value = "";
    loadWishes(); // โหลดใหม่หลังบันทึก
  }
});

// ==== โหลดข้อความทันทีตอนเปิดเว็บ ====
loadWishes();